...
const DEFAULT_INTERESTS: { label: string; categories: string[] }[] = [
  { label: 'Кофе и кафе', categories: ['Кафе'] },
  { label: 'Природа и парки', categories: ['Парки'] },
  { label: 'Красивые виды', categories: ['Виды'] },
  { label: 'История и архитектура', categories: ['Архитектура'] },
  { label: 'Музыка и клубы', categories: ['Музыка'] },
  { label: 'Всё сразу', categories: ['Кафе', 'Парки', 'Виды', 'Архитектура', 'Музыка'] },
];
...
export default function MapScreen() {
  ...
  const removeWaypoint = (id: string) => setWaypoints((prev) => prev.filter((p) => p.id !== id));
  ...
  const handleGenerate = async () => {
    ...
    try {
      const { data } = await routesApi.generate({
        ...
        transport_mode: transport, max_points: 5,
  ...
  const handleInterestsSubmit = async () => {
    ...
    try {
      ...
      router.push(`/route/${route.id}?preview=1`);
    } catch (e: any) {
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalWrap}>
          <View style={[styles.modalSheet, { backgroundColor: c.surface }]}>
            ...
            <View style={styles.modalHeader}>
              <Text style={[Typography.h3, { color: c.text1 }]}>Ваши интересы</Text>
              <TouchableOpacity onPress={() => setInterestsOpen(false)} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
                <XIcon color={c.text3} size={20} />
              </TouchableOpacity>
            </View>

            <Text style={[Typography.cap, { color: c.text3, marginBottom: 12 }]}>
              Расскажите что вам нравится — найдём подходящий маршрут
            </Text>

            <TextInput
              style={[styles.interestsInput, { backgroundColor: c.surface2, color: c.text1, borderColor: c.border }]}
              placeholder="Например: люблю кофе, гулять в парках и смотреть на старую архитектуру..."
              placeholderTextColor={c.text3}
              value={interestsText}
            ...
            <Text style={[Typography.cap, { color: c.text2, marginTop: 16, marginBottom: 8 }]}>
              Или выберите готовое
            </Text>

            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginHorizontal: -Spacing.screen }} contentContainerStyle={{ paddingHorizontal: Spacing.screen, gap: 8 }}>
              {DEFAULT_INTERESTS.map((preset) => {
                const isActive = preset.categories.every((cat) => activeCategories.includes(cat)) && preset.categories.length === activeCategories.length;
                return (
                  <TouchableOpacity
                    key={preset.label}
                    style={[styles.presetChip, { backgroundColor: isActive ? c.text1 : c.surface3, borderColor: isActive ? c.text1 : c.border }]}
                    onPress={() => {
                      setActiveCategories(preset.categories);
                      setInterestsOpen(false);
                    }}
                  >
                    <Text style={[Typography.cap, { color: isActive ? c.bg : c.text1 }]}>{preset.label}</Text>
                  </TouchableOpacity>
                );
              })}
            </ScrollView>
            ...
            <TouchableOpacity
              style={[styles.generateBtn, { backgroundColor: c.text1, marginTop: 20, opacity: interestsLoading || !interestsText.trim() ? 0.5 : 1 }]}
              onPress={handleInterestsSubmit}
              disabled={interestsLoading || !interestsText.trim()}
            >
              {interestsLoading ? <ActivityIndicator color={c.bg} /> : (
                <>
                  <SparkleIcon color={c.bg} />
                  <Text style={[Typography.bodyStrong, { color: c.bg, marginLeft: 10 }]}>Найти маршрут</Text>
                </>
              )}
            </TouchableOpacity>
          </View>
          ...
          <View>
            ...
            <View style={styles.waypointList}>
              {waypoints.map((wp, i) => (
                <View key={wp.id} style={styles.waypointItem}>
                  ...
                  <Text style={[Typography.body, { color: c.text1, flex: 1, marginHorizontal: 10 }]} numberOfLines={1}>
                    {wp.name}
                  </Text>
            ...
            >
              {loading ? <ActivityIndicator color={c.bg} /> : (
                <>
                  <SparkleIcon color={c.bg} />
                  <Text style={[Typography.bodyStrong, { color: c.bg, marginLeft: 10, marginRight: 6 }]}>
                    Построить маршрут
                  </Text>
          ...
          <View>
            <View style={styles.radiusHeaderRow}>
              <View>
                ...
                <Text style={[Typography.h1, { color: c.text1, fontSize: 42, lineHeight: 48 }]}>
                  {radiusKm.toFixed(1)}
...
const styles = StyleSheet.create({
  ...
  // Waypoints list
  waypointList: { marginTop: 12, gap: 8 },
